...
function CDelete_Click(PK)	{
	...
	{
	document.location="donor_registration_view.php?Action=Delete&pk="+PK+"&id="+PK;
	}
}
function CApprove_Click(PK)	{
document.location="donor_registration_view.php?Action=Approve&pk="+PK+"&id="+PK;
}

function CReject_Click(PK)	{
document.location="donor_registration_view.php?Action=Reject&pk="+PK+"&id="+PK;
}
</script>
<?php
// change-- query here
if($_REQUEST['Action'] =='Delete')
	...
	?>
<script>
window.location = 'donor_registration.php?delete=Record deleted sucessfully';
</script><?php }
...
?>
<script>
window.location = 'donor_registration.php?delete=Record Rejected sucessfully';
</script>
<?php }
if($_REQUEST['Action'] =='Approve'){
$id = $_REQUEST['pk'];
$sql =db_query("Update donor_register  set is_approve='1' where donor_register_id='".$_REQUEST['pk']."' ") or die(mysql_error);

$sql_search = db_query("select * from donor_register where donor_register_id='".$_REQUEST['pk']."'");
$data_collect=db_fetch_object($sql_search);
	$email = $data_collect->donor_email;
	
			$headers	=	"From: EspeciallytheFamily \r\n";
			$headers	.=	"Content-Type: text/html;\n";
			$msg1	=	'<table width="100%" border="0" cellspacing="2" cellpadding="2">';
			$msg1	.=	'<tr><td align="left" valign="top" colspan="3">Dear '.$data_collect->donor_username.'</tr>';
			$msg1	.=	'<tr><td align="left" valign="top" colspan="3" width="13%">You are now able to log in as an official ETF donor! Please contact us if you have any questions. Peace and love, Especially The Family.</td></tr>';
			$msg1	.=	'<tr><td align="left" valign="top" colspan="3" width="13%"><a href="'.BASEURL.'donorlogin.php">Click here!</a></td></tr>';
			$msg1	.=	'</table>';
			$sub	=	"Your Login Link :";
...
<input type="button" name="button" value="Approve"  onClick="JavaScript: CApprove_Click('<?php echo $r->donor_register_id; ?>');" style="background:url(images/login/inp_login.gif); color:#FFF; padding:2px; cursor:pointer; border:none" />
<?php  } ?>
<?php if($r->is_approve != '2') {?>
<input type="button" name="button" value="Reject"  onClick="JavaScript: CReject_Click('<?php echo $r->donor_register_id; ?>');" style="background:url(images/login/inp_login.gif); color:#FFF; padding:2px; cursor:pointer; border:none" />
<?php  } ?>
<input type="button" name="button" value="Delete"  onClick="JavaScript: CDelete_Click('<?php echo $r->donor_register_id; ?>');" style="background:url(images/login/inp_login.gif); color:#FFF; padding:2px; cursor:pointer; border:none" />                  
                  </div>
                  </td>
				  </tr>                  

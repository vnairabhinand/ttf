...
function CDelete_Click(PK)	{
	...
	{
	document.location="member_registration_view.php?Action=Delete&pk="+PK+"&id="+PK;
	}
}
function CApprove_Click(PK)	{
document.location="member_registration_view.php?Action=Approve&pk="+PK+"&id="+PK;
}

function CReject_Click(PK)	{
document.location="member_registration_view.php?Action=Reject&pk="+PK+"&id="+PK;
}
</script>
<?php
// change-- query here
if($_REQUEST['Action'] =='Delete')
	{
		$id = $_REQUEST['pk'];
		$sql = db_query("DELETE FROM  member_register  WHERE  member_register_id = ".$id) or die(mysql_error);
	?>
<script>
window.location = 'member_registration.php?delete=Record deleted sucessfully';
</script><?php }
if($_REQUEST['Action'] =='Reject'){
$id = $_REQUEST['pk'];
		$sql =db_query("Update member_register  set is_approve='2' where member_register_id='".$_REQUEST['pk']."' ") or die(mysql_error);
?>
<script>
window.location = 'member_registration.php?delete=Record Rejected sucessfully';
</script>
<?php }
if($_REQUEST['Action'] =='Approve'){
$id = $_REQUEST['pk'];
$sql =db_query("Update member_register  set is_approve='1' where member_register_id='".$_REQUEST['pk']."' ") or die(mysql_error);

$sql_search = db_query("select * from member_register where member_register_id='".$_REQUEST['pk']."'");
$data_collect=db_fetch_object($sql_search);
	$email = $data_collect->member_email;
	
			$headers	=	"From: EspeciallytheFamily \r\n";
			$headers	.=	"Content-Type: text/html;\n";
			$msg1	=	'<table width="100%" border="0" cellspacing="2" cellpadding="2">';
			$msg1	.=	'<tr><td align="left" valign="top" colspan="3">Dear '.$data_collect->member_username.'</tr>';
			$msg1	.=	'<tr><td align="left" valign="top" colspan="3" width="13%">You are now able to log in as an official ETF Church! Please contact us if you have any questions. Peace and love, Especially The Family.</td></tr>';
			$msg1	.=	'<tr><td align="left" valign="top" colspan="3" width="13%"><a href="'.BASEURL.'memberlogin.php">Click here!</a></td></tr>';
			$msg1	.=	'</table>';
			$sub	=	"Your Login Link :";
			$ret = mail($email, $sub, $msg1, $headers);
?>
<script>
...
$query = "select * from member_register where is_approve !='2' and member_register_id = '".$_SESSION['id']."' ";
$q=db_query($query);
$r=db_fetch_object($q);
?>
...
<div class="select_sub show">
			<ul class="sub">
				...
				<li class="sub_show"><a style="cursor:pointer"  href="member_registration.php">Pending Church Accounts <?php
...
<a href="member_registration.php" title="Back" class="info-tooltip"><h2>BACK</h2></a>	
</div>
			...
			<div id="table-content">
			  ...
			  <!--  start product-table ..................................................................................... -->
				...
				<tr>
			        ...
			        <h1 style="color:#FFF">Member Users Information</h1></div></th>
				  </tr>
             
				<tr>
					<td width="19%">Church Name</td>
					<td width="81%" align="left"><?php echo $r->church_name;?></td>
				  </tr>
				<tr>
				  <td><span class="inputleft">User Name</span></td>
				...
				<tr>
				  ...
				  <td align="left"><?php echo $r->member_first_name;?></td>
				  </tr>
				<tr>
				  <td><span class="inputleft">Last Name</span></td>
				  <td align="left"><span class="inputright"><?php echo $r->member_last_name;?></span></td>
				  </tr>
				<tr>
				  <td><span class="inputleft">Phone</span></td>
				  <td align="left"><span class="inputright">
				    <?php  echo $r->member_phone; ?>
				  </span></td>
				  </tr>
				<tr>
...
<input type="button" name="button" value="Approve"  onClick="JavaScript: CApprove_Click('<?php echo $r->member_register_id; ?>');" style="background:url(images/login/inp_login.gif); color:#FFF; padding:2px; cursor:pointer; border:none" />
<?php  } ?>
<?php if($r->is_approve != '2') {?>
<input type="button" name="button" value="Reject"  onClick="JavaScript: CReject_Click('<?php echo $r->member_register_id; ?>');" style="background:url(images/login/inp_login.gif); color:#FFF; padding:2px; cursor:pointer; border:none" />
<?php  } ?>
<input type="button" name="button" value="Delete"  onClick="JavaScript: CDelete_Click('<?php echo $r->member_register_id; ?>');" style="background:url(images/login/inp_login.gif); color:#FFF; padding:2px; cursor:pointer; border:none" />                  
                  </div>
                  </td>
				  </tr>                  

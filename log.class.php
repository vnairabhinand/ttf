...
class Logs
{
	...
	function logg($page=1,$msg=1,$priority='notSet',$color='blue',$mail='no',$uid=1,$UIDType=1){
		if($page == 1 || $msg == 1){
			if($page == 1){
		...
		return $this->addLog($msg,$_SESSION['username'],date('Y-m-d'),ip(),$priority,$color,$_SESSION['did'],$_SESSION['d'],$_SESSION['newamt'],$_SESSION['nedid'],date('Y-m-d'));

	
	}
	
		//the following function will log any activity in the pages with the code "$log->logg(parameters);" in them:
	function logg2($page=1,$msg=1,$priority='notSet',$color='blue',$mail='no',$uid=1,$UIDType=1){
		if($page == 1 || $msg == 1){
			if($page == 1){
				$page = $_SERVER['PHP_SELF']; //get full page direction (Ej. /index.php
				$pages = explode('/',$page); //explode the / and take 1 (Only suitable for first level pages)
		...
		return $this->addLog($msg,$_SESSION['username'],date('Y-m-d'),ip(),$priority,$color,$_SESSION['mid'],$_SESSION['m'],$_SESSION['newamt'],$_SESSION['nedid'],date('Y-m-d'));

	
	}
	
	function addLog($msg,$user,$timetamp,$ip,$priority,$class,$uiid,$UType,$newamt,$nedid,$todate){
		global $database;
		$sql = 'INSERT INTO `'.TBL_LOG.'` (`id`, `msg`, `user`, `timestamp`, `ip`, `priority`, `class`, `uid`, `UIDType`,`paypalamt`, `needid`,`paymentdate`) VALUES (\'\', \''.$msg.'\', \''.$user.'\', \''.$timetamp.'\', \''.$ip.'\', \''.$priority.'\', \''.$class.'\', \''.$uiid.'\', \''.$UType.'\', \''.$newamt.'\', \''.$nedid.'\', \''.$todate.'\')';
